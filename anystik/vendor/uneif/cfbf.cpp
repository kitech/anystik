...
void EifFile::unpackFile(const DirEntry& node, const fs::path& outPath) {
	...
	if (!fout) {
		throw IOError("Failed to create file " + std::string(outPath.u8string().begin(),
		                                                  outPath.u8string().end()));
	}
	char buf[SectSize];
	...
	if (!fout) {
		throw IOError("Failed to write file " + std::string(outPath.u8string().begin(),
		                                                   outPath.u8string().end()));
	}
}
